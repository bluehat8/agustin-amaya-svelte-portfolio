...
<div class="project-card relative overflow-hidden rounded-xl shadow">
  <div class="relative">
    <img
  ...
  <div class="project-info p-4">
    <h3 class="project-title text-lg font-semibold">{project.title}</h3>
    <p class="project-description text-sm opacity-80 mt-1">{project.description}</p>

    <div class="tags flex flex-wrap gap-2 mt-2">
      {#each project.tags as tag}
        <span class="tag bg-gray-200 dark:bg-gray-700 text-xs px-2 py-1 rounded-md">
          {tag}
        </span>
      {/each}
    </div>

    <div class="project-links flex gap-3 items-center mt-3">
      <a href={project.url} target="_blank" class="project-icon text-gray-800 dark:text-gray-200">
        <!-- icon -->
      ...
      {#if project.githubUrl}
        <a href={project.githubUrl} target="_blank" class="project-icon text-gray-800 dark:text-gray-200">
          <!-- icon -->
      ...
      {/if}
        ...
        {#if project.appUrl}
        <a href={project.appUrl} target="_blank" class="project-icon text-gray-800 dark:text-gray-200">
          <!-- icon -->
